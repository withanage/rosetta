...
class RosettaExportDeployment
{
		...
		if (is_dir($RosettaSubDirectory)) {
			...
			if (!is_dir($SIP_PATH)) {
				...
				foreach ($galleyFiles as $file) {
					...
					$copySuccess = copy($file["fullFilePath"], join(DIRECTORY_SEPARATOR, array($STREAM_PATH, $file["path"], basename($file["fullFilePath"]))));
					if (!$copySuccess) $failedFiles [] = $file["fullFilePath"];
					foreach ($file["dependentFiles"] as $dependentFile) {
						$copySuccess = copy($dependentFile["fullFilePath"], join(DIRECTORY_SEPARATOR, array($STREAM_PATH, $file["path"], basename($dependentFile["fullFilePath"]))));
						if (!$copySuccess) $failedFiles [] = $dependentFile["fullFilePath"];
					}
				}

				if (!$isTest and $this->getValidationStatus($IE_PATH) == 0 && count($failedFiles) == 0) {
	...
	public function getValidationStatus(string $IE_PATH): string
	{
